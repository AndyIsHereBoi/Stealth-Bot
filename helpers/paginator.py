...
class NodesMenu(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        ...
        embed = discord.Embed(title="<:rich_presence:895688440887246858> Node statistics", color=color)

        for i, v in enumerate(entries, start=offset):
            embed.add_field(name=''.join(v.keys()), value=''.join(v.values()), inline=True)
        return embed
