...
class QueueMenu(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        ...
        embed = discord.Embed(title=f"{f'**{len(self.data)}** songs in the queue' if len(self.data) != 1 else '**1** song in the queue'}", color=color)
        
        description = []
        
        for i, v in enumerate(entries, start=offset):
            description.append(f"{i+1}. {v}")
...
class NodesMenu(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        offset = menu.current_page * self.per_page
        embed = discord.Embed(title="<:rich_presence:895688440887246858> Node statistics")
...
class LyricsPageSource(menus.ListPageSource):
    def __init__(self, title: str, href: str, text: list, image: str, ctx):
        self.title = title
        self.href = href
        self.ctx = ctx
        self.text = text
    ...
    async def format_page(self, menu, entries):
        offset = menu.current_page * self.per_page
        embed = discord.Embed(title=self.title, url=self.href, description="\n".join(f'{data}' for i, data in enumerate(entries, start=offset)))
