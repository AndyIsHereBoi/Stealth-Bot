...
class Moderation(commands.Cog):
    ...
    async def temporary_mutes(self):
        ...
        if guild:
            ...
            mute_role = await self.client.db.fetchval('SELECT muted_id FROM prefixes WHERE guild_id = $1',
                                                   next_task['guild_id'])
            if mute_role:
...
New nickname: `{new}`
    ...
    @commands.has_permissions(manage_roles=True)
    @commands.bot_has_permissions(manage_roles=True)
    async def mute(self, ctx, member: discord.Member, *, reason: str = None) -> discord.Message:
        """
    ...
    @commands.has_permissions(manage_roles=True)
    @commands.bot_has_permissions(manage_roles=True)
    async def multi_mute(self, ctx, members: commands.Greedy[discord.Member],
    ...
    @commands.has_permissions(manage_roles=True)
    @commands.bot_has_permissions(manage_roles=True)
    async def hardmute(self, ctx, member: discord.Member, *, reason: str = None) -> discord.Message:
        """
    ...
    @commands.has_permissions(manage_roles=True)
    @commands.bot_has_permissions(manage_roles=True)
    async def multi_unmute(self, ctx, members: commands.Greedy[discord.Member], reason: str = None):
        """
        Mutes a lot of members indefinitely indefinitely.
        """
    ...
    @ensure_muterole()
    @commands.bot_has_permissions(manage_roles=True)
    async def selfmute(self, ctx, *, duration: time_inputs.ShortTime):
        """
        ...
        await self.client.db.execute("INSERT INTO temporary_mutes(guild_id, member_id, reason, end_time) "
    ...
    #     help="Changes the mute-role. If no role is specified it will show the current mute-role for this server.",
    #     aliases=['mute_role', 'mute-role'])
    # @commands.has_permissions(manage_roles=True)
    # @commands.bot_has_permissions(manage_roles=True)
    ...
    #             embed = discord.Embed(title="Mute-role updated!", description=f"The mute role has been updated to {role.mention}!", color=discord.Color.green())
