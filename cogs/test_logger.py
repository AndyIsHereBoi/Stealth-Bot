...
class LoggingBackend(commands.Cog):
    ...
    async def deliver_logs(self, embed: discord.Embed, guild: int):
        database = await self.client.db.fetchrow("SELECT * FROM guilds WHERE guild_id = $1", guild)
        try:
        ...
        except TypeError: # No record for that guild
            return

        except KeyError: # No logging channel set
            return

    @commands.Cog.listener('on_message_delete')
    async def logger_on_message_delete(self, message: discord.Message) -> discord.Message or None:
        if message.author.bot or not message.guild or message.guild.id not in self.client.logging_guilds:
            return

        embed = discord.Embed(title=f"Message deleted in #{message.channel}", description=(message.content or '\u200b')[0:4000], colour=discord.Colour.red(), timestamp=discord.utils.utcnow())
        ...
        await self.deliver_logs(embed, message.guild.id)

    @commands.Cog.listener('on_raw_bulk_message_delete')
    async def logger_on_raw_bulk_message_delete(self, payload: discord.RawBulkMessageDeleteEvent):
        if not payload.guild_id or payload.guild_id not in self.client.logging_guilds:
            return
        ...
        for message in payload.cached_messages:
            ...
            if message.attachments:
                ...
                attachment = f"Sticker: {message.stickers[0].name}"
            ...
            if len('\n'.join(messages)[0:4096]) > 4000:
                break

        embed = discord.Embed(title=f"{len(payload.message_ids)} messages purged in #{self.client.get_channel(payload.channel_id)}", description='\n'.join(messages)[0:4000], colour=discord.Colour.red(), timestamp=discord.utils.utcnow())
        embed.add_field(name="Showing: ", value=f"{len(messages)}/{len(payload.message_ids)} messages.", inline=False)
        embed.set_footer(text=f"Channel: {payload.channel_id}")
        await self.deliver_logs(embed, payload.guild_id)
