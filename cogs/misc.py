...
Guild owner: {ctx.guild.owner} **|** {ctx.guild.owner.mention} **|** {ctx.guild.owner.id}
    ...
    async def muterole(self, ctx, new_role: discord.Role = None):
        """
    ...
    async def muterole_remove(self, ctx):
        """
    ...
    @commands.has_permissions(manage_roles=True)
    @commands.bot_has_permissions(manage_roles=True)
    async def muterole_delete(self, ctx):
        """
        ...
        await self.client.db.execute(
            "INSERT INTO guild(guild_id, muted_role_id) VALUES ($1, $2) "
            "ON CONFLICT (guild_id) DO UPDATE SET muted_role_id = $2",
            ctx.guild.id, None)
        await ctx.send("🚮")
    ...
    @commands.has_permissions(manage_roles=True)
    @commands.bot_has_permissions(manage_roles=True)
    async def muterole_fix(self, ctx):
        async with ctx.typing():
            starting_time = time.monotonic()
            mute_role = await self.client.db.fetchval('SELECT muted_role_id FROM guilds WHERE guild_id = $1', ctx.guild.id)
            ...
            for channel in ctx.guild.channels:
                perms = channel.overwrites_for(role)
                perms.update(send_messages=False, add_reactions=False, connect=False, speak=False, create_public_threads=False, create_private_threads=False, send_messages_in_threads=False, )
                try:
                    await channel.set_permissions(role, overwrite=perms,
                                                  reason=f"DuckBot mute-role creation. Requested "
                                                         f"by {ctx.author} ({ctx.author.id})")
